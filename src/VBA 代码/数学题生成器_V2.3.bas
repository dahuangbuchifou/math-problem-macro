...
' ==================== 页面布局常量 ====================
Const COLS_PER_PAGE As Integer = 25    ' 每页列数（横向 25 道题）
...
Sub GenerateQuestions()
    ...
    Dim rowsPerPage As Integer       ' V2.3: 每页行数（动态）
    Dim questionsPerPage As Integer  ' V2.3: 每页题目数（动态 = 25 × rowsPerPage）
    Dim targetRowHeight As Integer   ' V2.3: 目标行高（根据 rowsPerPage 自适应）
    Dim p As Integer
    ...
    rowsPerPage = Val(wsQuestion.Range("H8").Value)
    ...
    wsQuestion.Range("H8").Value = rowsPerPage
    wsQuestion.Range("H9").Value = printMode
    
    ' ============================================================
    ' V2.3: 根据 rowsPerPage 计算每页题目数
    ' ============================================================
    ' 校验 rowsPerPage
    If rowsPerPage < 2 Then rowsPerPage = 2
    If rowsPerPage > 4 Then rowsPerPage = 4
    
    questionsPerPage = COLS_PER_PAGE * rowsPerPage  ' 25×2=50, 25×3=75, 25×4=100
    ...
    ' 计算总行数（每页 rowsPerPage 行 + 页间分隔行）
    totalRows = totalPages * rowsPerPage + (totalPages - 1)
    ...
    With wsQuestion
        ...
        ' 设置行高（V2.3: 根据 rowsPerPage 自适应）
        Select Case rowsPerPage
            Case 2: targetRowHeight = 25
            Case 3: targetRowHeight = 22
            Case 4: targetRowHeight = 18
    ...
    For i = 1 To questionCount
        ...
        ' 计算在工作表中的列和行
        col = ((pageQuestionIndex - 1) Mod COLS_PER_PAGE) + 1
        rowInPage = ((pageQuestionIndex - 1) \ COLS_PER_PAGE) + 1
        
        ' 考虑分隔行偏移
        row = rowInPage + (currentPage - 1)
