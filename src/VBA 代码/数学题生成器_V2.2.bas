...
Sub GenerateQuestions()
    ...
    If questionCount > QUESTIONS_PER_PAGE * MAX_PAGES Then questionCount = QUESTIONS_PER_PAGE * MAX_PAGES
    If totalPages < 1 Then totalPages = 1
    If totalPages > MAX_PAGES Then totalPages = MAX_PAGES
    
    ' 根据页数调整题目数量（如果用户没手动改）
    If totalPages > 1 And questionCount < totalPages * QUESTIONS_PER_PAGE Then
        questionCount = totalPages * QUESTIONS_PER_PAGE
    End If
    ...
    ' 计算总行数（每页 ROWS_PER_PAGE 行 + 页间分隔行）
    ...
    For i = 1 To questionCount
        ' 计算当前题目所在的页码和页内索引
        currentPage = ((i - 1) \ QUESTIONS_PER_PAGE) + 1
        pageQuestionIndex = ((i - 1) Mod QUESTIONS_PER_PAGE) + 1
